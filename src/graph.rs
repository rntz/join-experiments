...
pub fn edge_db(edges: &[(usize, usize)]) -> VecDb {
    let rows: Vec<Vec<Value>> = edges.iter().map(|&(a, b)| row![a, b]).collect();
...
pub fn binary_triangles_directed(edges: &[(usize, usize)]) -> Vec<Vec<usize>> {
    let edge_set: Set<(usize, usize)> = edges.iter().copied().collect();
    let mut out: Map<usize, Vec<usize>> = Map::default();
...
// edges a -> b -> c, a -> c.
pub fn to_low_high(edges: &[(usize, usize)]) -> Vec<(usize, usize)> {
    let mut v: Vec<(usize, usize)> = edges.iter()
        .filter(|&&(a, b)| a != b)
...
pub fn symmetrize(edges: &[(usize, usize)]) -> Vec<(usize, usize)> {
    let mut v: Vec<(usize, usize)> = edges.iter()
        .filter(|&&(a, b)| a != b)
...
pub fn binary_triangles_undirected(edges: &[(usize, usize)]) -> Vec<Vec<usize>> {
    let edge_set: Set<(usize, usize)> = edges.iter().copied().collect();
    let mut out: Map<usize, Vec<usize>> = Map::default();
    for &(a, b) in edges { out.entry(a).or_default().push(b); }
    let mut triangles: Vec<Vec<usize>> = Vec::new();
